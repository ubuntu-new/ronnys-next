...
export async function startProduction(fd: FormData) {
  ...
  if (!recipeId || !locationId) throw new Error("აირჩიე რეცეპტი და ლოკაცია");
  if (batches === null || batches <= 0) throw new Error("გატარებების რაოდენობა ნულზე მეტი უნდა იყოს");
  ...
  if (recipe.lines.length === 0) throw new Error("რეცეპტს შემავალები არ აქვს");
...
export async function finishProduction(id: string, fd: FormData) {
  ...
  if (!order) throw new Error("პარტია ვერ მოიძებნა");
  if (order.status !== "in_progress") throw new Error("პარტია უკვე დახურულია");

  const actualQty = fdNum(fd, "actualQty");
  if (actualQty === null || actualQty < 0) throw new Error("შეიყვანე ფაქტობრივი გამოსავალი");
  ...
  for (const l of order.lines) {
    ...
    if (used < 0) throw new Error("დახარჯული უარყოფითი ვერ იქნება");
  ...
  await logAction({
    ...
    entityType: "ProductionOrder",
    entityId: id,
    after: { no: order.no, planned, actual: actualQty, გამოსავალი: yieldPct ? `${yieldPct}%` : null, used: usedLog },
    employeeId: s.sub,
  });
